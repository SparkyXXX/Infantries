...
{
    BoardCom_Reset();
		BoardCom_Data.cap_rest_energy = 45;
    FDCAN_InitTxHander(&BoardCom_GimControl, SET_CONTROL, FDCAN_DLC_BYTES_8, FDCAN_BRS_OFF, FDCAN_CLASSIC_CAN);
    FDCAN_InitTxHander(&BoardCom_GimImuYaw, SET_IMU_YAW, FDCAN_DLC_BYTES_8, FDCAN_BRS_OFF, FDCAN_CLASSIC_CAN);
    FDCAN_InitTxHander(&BoardCom_GimChassisRef, SET_CHA_REF, FDCAN_DLC_BYTES_8, FDCAN_BRS_OFF, FDCAN_CLASSIC_CAN);
    FDCAN_InitTxHander(&BoardCom_GimUIState, SET_UI_STATE, FDCAN_DLC_BYTES_8, FDCAN_BRS_OFF, FDCAN_CLASSIC_CAN);
    FDCAN_InitTxHander(&BoardCom_ChaRefereeData1, SET_REFEREE_DATA_1, FDCAN_DLC_BYTES_8, FDCAN_BRS_OFF, FDCAN_CLASSIC_CAN);
    FDCAN_InitTxHander(&BoardCom_ChaRefereeData2, SET_REFEREE_DATA_2, FDCAN_DLC_BYTES_8, FDCAN_BRS_OFF, FDCAN_CLASSIC_CAN);
    FDCAN_InitTxHander(&BoardCom_CapMode, SET_CAP_MODE, FDCAN_DLC_BYTES_8, FDCAN_BRS_OFF, FDCAN_CLASSIC_CAN);
    FDCAN_InitTxHander(&BoardCom_CapState1, SET_CAP_DATA_1, FDCAN_DLC_BYTES_8, FDCAN_BRS_OFF, FDCAN_CLASSIC_CAN);
    FDCAN_InitTxHander(&BoardCom_CapState2, SET_CAP_DATA_2, FDCAN_DLC_BYTES_8, FDCAN_BRS_OFF, FDCAN_CLASSIC_CAN);
}
...
void BoardCom_ChassisModeSet()
{
    ...
    case CHASSIS_CTRL_BACKGYRO:
    {
        Chassis_ModeSet(CHASSIS_BACKGYRO);
        Chassis_SetMoveRef(boardcom->chassis_fb_ref, boardcom->chassis_lr_ref);
        break;
    }
    default:
...
static void _send_referee_data_1(uint8_t buff[])
{
    ...
    ui162buff(boardcom->heat_now, buff + 6);
...
static void _set_control(uint8_t buff[])
{
    ...
    boardcom->ui_cmd = (buff[1] & 0x04) >> 2;
		...
		static uint8_t Lowpower_Protect_Flag;
    ...
    if (cap->rest_energy < 15){
		...
		if(Lowpower_Protect_Flag == 1){	
			if(boardcom->cap_mode_user == SUPERCAP_CTRL_ON){
        boardcom->power_path_change_flag = HAL_GetTick();
			}
...
static void _set_cap_data_1(uint8_t buff[])
{
    ...
    boardcom->cap_rest_energy = buff[4];
		...
		boardcom->boost_version = (buff[5]&0xF0) >> 4;//lzq
