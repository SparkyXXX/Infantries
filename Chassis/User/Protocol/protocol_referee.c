...
void Referee_Init(UART_HandleTypeDef *huart)
{
    ...
    referee->huart = huart;
    UART_InitDMA(referee->huart);
    UART_ReceiveDMA(referee->huart, Referee_RxData, REFEREE_RX_BUFF_LEN);
...
//    if (referee->robot_level < 1 || referee->robot_level > 10)
//    {
//        referee->robot_level = 1;
//    }

//    uint8_t is_error_maxpower = 1;
//    for (int8_t i = 0; i < 2; i++)
//    {
//        if (referee->chassis_power_limit == level_maxpower_table[referee->robot_level - 1][i])
//        {
//            is_error_maxpower = 0;
//            break;
//        }
//    }
//    if (is_error_maxpower == 1)
//    {
//        referee->chassis_power_limit = level_maxpower_table[referee->robot_level - 1][1];
//    }
	referee_power_limit_watch = referee->chassis_power_limit;
...
uint8_t P_ext_power_heat_data(Referee_DataTypeDef *referee, void *data_ptr)
{
	...
	buffer_energy_watch = referee->buffer_energy;

// hrx:异常数据数值测定
//    if (referee->buffer_energy > 50 && cap->rest_energy < 90)
//    {
//        referee->buffer_energy = 30;
//    }
