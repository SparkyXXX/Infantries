...
void PID_Init(PID_TypeDef *pid, PID_ModeEnum mode, float kp, float ki, float kd,
              float kf_1, float kf_2, float kf_3, float sum_max, float output_max,
              float kd_filter, float kf1_filter, float kf2_filter, float kf3_filter)
{
    pid->pid_mode = mode;
    pid->kp = kp;
    pid->ki = ki;
	...
	Filter_Lowpass_Init(kf3_filter, &pid->kf3_filter);
}
...
float PID_Calc(PID_TypeDef *pid)
{
    if (pid->pid_mode == PID_POSITION)
    {
        ...
        pid->output_fdf = Filter_Lowpass((pid->kf_1 * ref_dError), &pid->kf1_filter) + Filter_Lowpass((pid->kf_2 * ref_ddError), &pid->kf2_filter) + Filter_Lowpass((pid->kf_3 * pid->ref), &pid->kf3_filter);
