...
static void _send_control(uint8_t buff[])
{
    ...
    buff[1] = (boardcom->check_in << 6) + (minipc->is_get_target << 5) + (boardcom->gyro_dir << 4) + (boardcom->fly_flag << 3) + (boardcom->ui_cmd << 2) + (boardcom->cap_mode_user << 1) + boardcom->cap_speedup_flag;
