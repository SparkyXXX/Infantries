...
void MiniPC_Send()
{
    ...
    GPIO_Reset(&TRIGGER);
		if(boardcom->yaw_mode == GIMBAL_YAW_BIG_ENERGY || boardcom->yaw_mode == GIMBAL_YAW_SMALL_ENERGY){
			osDelay(38);//1000/(38+2)=25hz
		}else{
			osDelay(8);//1000/(8+2)=100hz
		}
}

/*uint16_t watch_trigg_freq,trigg_i;
static uint32_t last_zero_tick;
		trigg_i ++;
		if(HAL_GetTick() - last_zero_tick > 1000){
			watch_trigg_freq = trigg_i;
			trigg_i = 0;
			last_zero_tick = HAL_GetTick();
		}*/
