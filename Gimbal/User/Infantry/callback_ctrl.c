...
void UART_ReceiveHandler(UART_HandleTypeDef *huart)
{
    if (__HAL_UART_GET_FLAG(huart, UART_FLAG_IDLE) && __HAL_UART_GET_IT_SOURCE(huart, UART_IT_IDLE))
    {
        ...
        if (huart == &huart2)
        {
            ...
            __HAL_DMA_DISABLE(huart->hdmarx);
            int rxdatalen = Const_ext_Remote_RX_BUFF_LEN - UART_DMACurrentDataCounter(huart->hdmarx);
            extRC_DecodeExtRemoteData(ext_rc->rxdata,rxdatalen);
            __HAL_DMA_SET_COUNTER(huart->hdmarx, Const_ext_Remote_RX_BUFF_LEN);
            __HAL_DMA_ENABLE(huart->hdmarx);
        }
    }
}
