 ...
 * @LastEditTime: 2024-05-25 11:53:24
 */
    ...
    if (autoaim->AutoShootFlag == 1)
    {
        if (autoaim->hit_mode == AUTOAIM_HIT_ARMOR)
        {
            ...
            if (minipc->is_get_target == 1 && minipc->is_shoot == 1 && autoaim->AutoShootFlag == 1 && wait_tick >= wait_ms &&
                (shooter->heat_ctrl.shooter_heat_limit - shooter->heat_ctrl.shooter_heat_now) >= 40 && Motor_shooterMotorLeft.encoder.speed > 20 && Motor_shooterMotorRight.encoder.speed > 20)
            {
