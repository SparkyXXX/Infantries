/* USER CODE BEGIN Header */
/**
 ******************************************************************************
 * @file           : main.h
 * @brief          : Header for main.c file.
 *                   This file contains the common defines of the application.
 ******************************************************************************
 * @attention
 *
 * Copyright (c) 2022 STMicroelectronics.
 * All rights reserved.
 *
 * This software is licensed under terms that can be found in the LICENSE file
 * in the root directory of this software component.
 * If no LICENSE file comes with this software, it is provided AS-IS.
 *
 ******************************************************************************
 */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32g4xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */
/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define CS1_GYRO_Pin GPIO_PIN_14
#define CS1_GYRO_GPIO_Port GPIOC
#define CS1_ACCEL_Pin GPIO_PIN_15
#define CS1_ACCEL_GPIO_Port GPIOC
#define Encoder_Right_A_Pin GPIO_PIN_0
#define Encoder_Right_A_GPIO_Port GPIOA
#define Encoder_Right_B_Pin GPIO_PIN_1
#define Encoder_Right_B_GPIO_Port GPIOA
#define BMI088_SCK_Pin GPIO_PIN_5
#define BMI088_SCK_GPIO_Port GPIOA
#define BMI088_MISO_Pin GPIO_PIN_6
#define BMI088_MISO_GPIO_Port GPIOA
#define BMI088_MOSI_Pin GPIO_PIN_7
#define BMI088_MOSI_GPIO_Port GPIOA
#define Snail_Left_Pin GPIO_PIN_0
#define Snail_Left_GPIO_Port GPIOB
#define Snail_Right_Pin GPIO_PIN_1
#define Snail_Right_GPIO_Port GPIOB
#define ADC_Snail_Pin GPIO_PIN_2
#define ADC_Snail_GPIO_Port GPIOB
#define TIM_Ammo_Cover_Pin GPIO_PIN_14
#define TIM_Ammo_Cover_GPIO_Port GPIOB
#define TRIGGER_Pin GPIO_PIN_15
#define TRIGGER_GPIO_Port GPIOB
#define Encoder_Left_B_Pin GPIO_PIN_8
#define Encoder_Left_B_GPIO_Port GPIOA
#define Encoder_Left_A_Pin GPIO_PIN_9
#define Encoder_Left_A_GPIO_Port GPIOA

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
